...
const ImageList = () =>{
    ...
    const [query, setQuery] = useState('bike tour')
    ...
    const lastPicElementRef = useCallback((node = lastPicture) => {
        if (loading) return
        if (observer.current) observer.current.disconnect()
        observer.current = new IntersectionObserver(entries => {
            
        })
        if(node) observer.current.obser
        console.log(node)
    })
            ...
            .then((data) =>{
                ...
                let pics = photo.map((pic,index) =>{
                    let srcPath = `https://farm${pic.farm}.staticflickr.com/${pic.server}/${pic.id}_${pic.secret}.jpg`;
                        if(pic.length === index + 1){
                            return <ImageCard ref={lastPicElementRef} key={srcPath} alt="" src={srcPath} />
