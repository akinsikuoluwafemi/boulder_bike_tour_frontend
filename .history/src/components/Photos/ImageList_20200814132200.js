...
const ImageList = () =>{
    ...
    const [totalPages, setTotalPages] = useState(null)
    const [query] = useState('cyclist')
    let [hasMore, setHasMore] = useState(pageNumber > 1)
    // const imageRef = useRef();
    ...
    //            return <img ref={lastImageRef} key={index} alt="" src={srcPath} />        
                 
    //         }else {
    //             return <img alt="" key={index} src={srcPath} />        
    ...
    const LoadMorePictures = () => {
        return new Promise((resolve, reject) => {
            const flickrapikey = '162e01778853d65e29516a0b540192d9';
            fetch('https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=' + flickrapikey + `&tags=${query}&per_page=${perPage}&page=${pageNumber}&format=json&nojsoncallback=1`)
                .then(response => response.json())
                .then((data) => {
        ...
        // axios.get('https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=' + flickrapikey + `&tags=${query}&per_page=${perPage}&page=${pageNumber}&format=json&nojsoncallback=1`)
        //     .then(response => {
                
        //         let { pages, photo } = response.data.photos
        ...
        //         let newPicturesArray = [...pictures, ...pics]
        //         console.log(pictures, newPicturesArray)
    ...
    return (
        <div className="py-4">
            ...
            <div>
                ...
                <div  onClick={addPictures} className="text-center m-auto example_d">LOAD MORE</div>
            </div>
